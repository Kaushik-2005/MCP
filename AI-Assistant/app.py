...
async def run_memory_chat():
    ...
    llm = ChatGroq(model = "qwen-qwb-32b")

    # Create an agent with memore_enabled = True
    agent = MCPAgent(
        llm = llm,
        client = client,
        max_steps = 15,
        memory_enabled = True, # Enable built-in conversation history
    )
    ...
    try:
        ...
        while True:
            ...
            try: 
                # RUn the agent with user input (memory handling in automatic)
